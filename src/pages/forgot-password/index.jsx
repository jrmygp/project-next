...
const ForgotPassword = () => {
    const formik = useFormik({
        ...
        validationSchema: yup.object().shape({
            email: yup.string().email("Please use a valid email").required("This field is required!")
        }),
    ...
    <Flex background="black">
      ...
      >
        ...
        <FormControl paddingX={20} isInvalid={formik.errors.email}>
          ...
          <Input id="inputEmail" name="email" onChange={inputHandler} color="white"/>
          <FormHelperText>{formik.errors.email}</FormHelperText>
        </FormControl>
        <Box paddingX="20px" marginTop={5}>
            <Link href={"/login"}>
            <Button colorScheme="blue" mr={5}>Back</Button>
            </Link>
          <Button colorScheme="green" onClick={formik.handleSubmit}>Send Email</Button>
        </Box>
      </Flex>
    </Flex>
  );
